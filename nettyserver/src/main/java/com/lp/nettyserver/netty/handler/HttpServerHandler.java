...
public class HttpServerHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        log.info("http通道激活，用户：{}，{}",ctx.channel().id(), DateFormatUtils.format(new Date(),"yyyy-MM-dd HH:mm:ss") );
    ...
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        log.info("http通道不活跃，用户：{}，{}",ctx.channel().id(), DateFormatUtils.format(new Date(),"yyyy-MM-dd HH:mm:ss"));
    ...
    protected void channelRead0(ChannelHandlerContext channelHandlerContext, FullHttpRequest fullHttpRequest) {
        log.info("http收到用户{}的{}请求",channelHandlerContext.channel().id(),fullHttpRequest.method());
